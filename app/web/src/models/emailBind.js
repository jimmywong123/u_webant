...
export default {
  ...
  effects: {
    ...
    *bind({ payload: { email, captcha } }, { call, put }){
      const { data: { errSms = '', succeed } } = yield call(bindEmail, { email, captcha } );
      if (succeed) {
    ...
    *unbind({ payload: { email } }, { call, put }){
      const { data: { succeed, errSms }}  = JSON.parse(yield call(unbindEmail, email));
      if (succeed) {
        message.success(intl.get('succeed'));
      } else if (errSms) {
        message.error(intl.get(errSms));
