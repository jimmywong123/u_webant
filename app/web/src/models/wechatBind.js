...
export default {
  ...
  reducers: {
    fetchWechatuser(state, { payload: { wechatuser }}) {
  ...
  effects: {
    ...
    *bind({ payload: { code, which } }, { call, put }){
      const fb = which === 'wechat' ? bindWechat : bindWebapp;
      const { data: { errSms = '', succeed } } = yield call(fb, code );
      if (succeed) {
    ...
    *unbind({ payload }, { call, put }){
      const { data: { succeed, errSms }}  = JSON.parse(yield call(unbindWechat, payload));
      if (succeed) {
        message.success(intl.get('succeed'));
      } else if (errSms) {
        message.error(intl.get(errSms));
