...
class HeaderView extends PureComponent {
  ...
  getLoginPageUrl() {
    getSystemPath('loginweb').then( res => {
      ...
      const loginPageUrl = redirectUrlParams.origin === data ? `${data}/user/login` : `${data}/user/login?${stringify({redirect})}`;
      const settingUrl = redirectUrlParams.origin === data ? `${data}/setting` : `${data}/setting?${stringify({redirect})}`;
      const updateState = {
        initDone: true,
        loginPageUrl,
        settingUrl
  ...
  handleNoticeClear = noticeKey => {
    message.success(
      `${intl.get('cleared')} ${ noticeKey ? intl.get(noticeKey) : ''}`
    );
