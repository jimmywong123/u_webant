...
class MainMenu extends Component {
  ...
  getMenuItem = (item, i) => {
    if (item.menuItemGroup) {
      ...
      return (
        <SubMenu key={`subMenu${i}`} title={<span className="submenu-title-wrapper">{item.icon && <Icon type={item.icon} />} {intl.get(item.titleKey)}</span>}>
          {
      ...
      return (
        <Menu.Item key={item.key}>
          <a {...item.url}>{item.icon && <Icon type={item.icon} />} {intl.get(item.titleKey)}</a>
        </Menu.Item>
      )
    } 
      return (
    ...
    return (
      ...
      <Header className={style.header}>
        ...
        > 
          { 
            leftMenuLsit && (
              leftMenuLsit.map( (item) => this.getMenuItem(item))
            )
          }
        </Menu>
        <div className="logo" />
        ...
        > 
          {
            rightMenuLsit && ( 
              rightMenuLsit.map( (item) => this.getMenuItem(item))
            )
          }
          <Menu.Item key={canChangeLang}>
            <a href={`?lang=${canChangeLang}`}><Icon type="global" />{intl.get(canChangeLang)}</a>
          </Menu.Item>
        </Menu>
